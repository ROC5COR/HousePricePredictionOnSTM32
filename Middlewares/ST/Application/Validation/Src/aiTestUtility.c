 ...
 *  - v1.2 - Adding STM32H7/L3 MCU support
 *
 */
...
#ifdef _IO_OUT_PACKET_MODE  /* Experimental */

#include <stm32msg.pb.h>


#define _PACKET_PAYLOAD_SIZE EnumLowLevelIO_IO_IN_PACKET_SIZE

static struct packet {
    uint8_t pw;
    uint8_t payload[_PACKET_PAYLOAD_SIZE];
} o_packet;
...
static bool write_packet(void) {
    ...
    return (status == HAL_OK ? (_PACKET_PAYLOAD_SIZE + 1) : 0);
}
        ...
        size_t count)
{
    ...
    while (count) {
        for (; o_packet.pw < _PACKET_PAYLOAD_SIZE && count; o_packet.pw++) {
            o_packet.payload[o_packet.pw] = *pr;
            pr++;
            count--;
        }
        if (o_packet.pw == _PACKET_PAYLOAD_SIZE) {
                res = write_packet();
                o_packet.pw = 0;
        }
    }
